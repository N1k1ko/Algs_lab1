//LongCount.longCount("999", "+", "15"); //1014
class Exsr3
{
    static string Add(string num1, string num2, int carry)
    {
    ...
    static string PostSubtract(string num1, string num2) //удаляет 0 из начала при вычитании
    {
        var result = Subtract(num1, num2);
        return result[0] == '0' ? result[1..] : result;
    }

    static string PerformOperation(string num1, string op, string num2)
    {
        var ch1 = num1[0] == '-' ? '-' : '+';
        var ch2 = num2[0] == '-' ? '-' : '+';
            ...
            case "+":
                {
                    switch (ch1, ch2)
                    {
                        case ('-', '-'): return "-" + Add(num1[1..], num2[1..], 0);
                        case ('+','+'):  return Add(num1, num2, 0);
                        case ('-', '+'):
                        {
                            if (len1 >= len2 && num1[1] > num2[0])
                                return "-" + PostSubtract(num1,num2);
                            else 
                                return PostSubtract(num1,num2);
                        }
                        case ('+','-'): return PostSubtract(num1,num2);
                    }
                    break;
                }
            case "-":
                {
                    switch (ch1, ch2)
                    {
                        case ('+', '-'): return Add(num1, num2[1..], 0);
                        case ('-', '+'): return "-" + Add(num1[1..], num2, 0);
                        case ('-','-'): return PostSubtract(num1,num2);
                        case ('+','+'): return PostSubtract(num1,num2);
                    }
                    break;
